...
namespace SF {
  ...
  StatusResult InsertStatement::parseValues(Parser &aParser, Schema &aSchema) {
    StatusResult theResult{true};
    ...
    while(aParser.peekToken(theToken)) {
      if(TokenType::lparen==theToken.type) {
        ...
        if(theResult) {
          ...
          KeyValueList *theList = new KeyValueList;
          for(int i=0;i<fields.size();i++) {
            try {
              ...
              theList->push_back(KeyValuePair(fields[i].name, theValue));
            }
            catch(...) {
            }
          }
    ...
    if(theResult) {
      ...
      if(aParser.peekToken(theToken)) {
        if(Keywords::values_kw==theToken.keyword) {
          aParser.skip(1);
          theResult=parseValues(aParser, aSchema);
        }
      }
    }
    return theResult;
  ...
  StatusResult InsertStatement::validate(Schema &aSchema) const {
    ...
    if(fsize && vsize) {
      ...
      for (auto theList : values) {
        if(fsize!=theList->size() || !aSchema.validate(*theList)) {
  ...
  KeyValueList* InsertStatement::getValueGroupAt(int anIndex) {
